...
ALTER TABLE `tickets` ADD `EmailId` BIGINT NULL AFTER `Title`;
ALTER TABLE `tickets` ADD CONSTRAINT `Email_Id` FOREIGN KEY (`EmailId`) REFERENCES `emails`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
ALTER TABLE `tickets` ADD `UserId` INT NULL COMMENT 'Audience user' AFTER `SenderEmail`;
ALTER TABLE `tickets` ADD CONSTRAINT `User_Id` FOREIGN KEY (`UserId`) REFERENCES `users`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `tickets` ADD `IsUserReceiver` BIT(1) NOT NULL DEFAULT b'0' COMMENT 'If admin opens a ticket with user' AFTER `AdminId`;
ALTER TABLE `tickets` ADD CONSTRAINT `Department_Id` FOREIGN KEY (`DepartmentId`) REFERENCES `departments`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
CREATE TABLE `departments` ( `Id` INT NOT NULL AUTO_INCREMENT , `Name` VARCHAR(3000) NOT NULL , `IsPrivate` BIT(1) NOT NULL DEFAULT b'0' , PRIMARY KEY (`Id`)) ENGINE = InnoDB;

CREATE TABLE `staffs` ( `UserId` INT NOT NULL , `DepartmentId` INT NOT NULL ) ENGINE = InnoDB;
ALTER TABLE `staffs` ADD CONSTRAINT `Staff_User` FOREIGN KEY (`UserId`) REFERENCES `users`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE `staffs` ADD CONSTRAINT `Staff_Department` FOREIGN KEY (`DepartmentId`) REFERENCES `departments`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
